# {{ site.data.name }}
A minimal, single-user deployment of [Eclipse {{ site.data.name }}](http://www.eclipse.org/orion/). {{ site.data.name }} provides an extensible IDE that runs in your browser. It's particularly good for writing Javascript.
...
### Running the server
1. Browse to the directory where you installed {{ site.data.name }}, usually `node_modules/orion/`.
2. Edit the `orion.conf` file. Uncomment the following line, replacing the password with something of your choice:
  ```
  pwd=secr3tPassw0rd
  ```
  This prevents unauthorized access to your {{ site.data.name }} server.
3. Run `npm start orion` or `node [node_modules]/orion/server.js`.
4. Go to **[http://localhost:8081](http://localhost:8081)** to use {{ site.data.name }}. You will be prompted with a basic auth dialog; enter the password you chose earlier.

The port number can be changed by passing the `--port` argument or setting the `PORT` environment variable.

### Global installation
If you installed {{ site.data.name }} as a global package with `npm install -g orion`, you can run the command `orion` from any directory (rather than `npm start orion` as outlined above.)
{{ site.data.name }} will then start using your current directory (or a directory you specify on the command  line) as the location.
...
### Using {{ site.data.name }} within a larger app
