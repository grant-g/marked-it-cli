		...
		var generateTOCs = function(source, destination) {
			...
			if (tocFileLines) {
				var generateTOC = function(adapter, tocFilename, xmlMode) {
					...
					var rootElement = common.htmlToDom("<root></root>", {xmlMode: xmlMode});
					...
					for (var i = 0; i < tocFileLines.length; i++) {
						...
						if (topicsString !== newTopicsString) {
							newTopics = common.htmlToDom(newTopicsString, {xmlMode: true});
						}
				...
				if (tocDITA) {
					...
					tocContent = generateTOC(tocDITAadapter, FILENAME_TOC_DITA, true);
					writeTocFile(tocFilename, tocContent, logger, "dita.toc.onComplete");
				}
				if (tocHTML) {
					tocFilename = path.join(destination, FILENAME_TOC_HTML);
					tocContent = generateTOC(tocHTMLadapter, FILENAME_TOC_HTML, false);
					writeTocFile(tocFilename, tocContent, logger, "html.toc.onComplete");
				}
			}
		}
