...
var STARTDELIMITER_FRONTMATTER = "---\n";
var ENDDELIMITER_FRONTMATTER = "\n---\n";
...
function extractFrontMatter(text) {
	if (text.indexOf(STARTDELIMITER_FRONTMATTER) !== 0) {
		return null; /* no valid start delimiter */
	}

	var endBound = text.indexOf(ENDDELIMITER_FRONTMATTER, STARTDELIMITER_FRONTMATTER.length);
	if (endBound === -1) {
		return null; /* no end delimiter */
	}

	var frontMatter = text.substring(STARTDELIMITER_FRONTMATTER.length, endBound);
	var map = jsYaml.safeLoad(frontMatter);
	return {
		map: map,
		text: text.substring(endBound + ENDDELIMITER_FRONTMATTER.length)
	}
}

function replaceVariables(text, siteDataMap, localMap) { /* conref variables */
