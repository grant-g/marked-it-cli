...
function generate(options, logger) {
	...
	if (extensionFiles && extensionFiles.length) {
		...
		extensionFiles.forEach(function(current) {
			try {
				var extensionPath = fs.realpathSync(current);
	...
	function traverse_tree(source, destination) {
		...
		for (var i = 0; i < filenames.length; i++) {
			...
			if (stat.isDirectory()) {
				...
				if (tocOrder) {
					function addLink(tocsArray, tocFilename) {
						...
						try {
							...
							if (stat.isFile()) {
								var outputPath = path.join(filename, tocFilename);
								var linkElementText = '<toc><link toc="' + outputPath + '"></link></toc>\n';
								var index = tocOrder.indexOf(filename);
								if (index !== -1) {
				...
				if (extension === EXTENSION_MARKDOWN) {
					...
					if (tocOrder) {
						index = tocOrder.indexOf(filename);
						if (index !== -1) {
							if (result.xmlToc) {
								var xmlTocDom = common.htmlToDom(result.xmlToc.text, {xmlMode: true});
								while (xmlTocDom && !(xmlTocDom.name && xmlTocDom.name.toLowerCase() === "toc")) {
									xmlTocDom = xmlTocDom.next;
								}
								if (xmlTocDom) {
									xmlTocs[index] = common.domToHtml(xmlTocDom, {xmlMode: true});
								} else {
									logger.warning("Excluded from XML TOC file because file TOC does not contain a <toc>: " + destinationPath);
								}
							}
							if (result.htmlToc) {
							...
							if (result.ditaToc) {
								var ditaTocDom = common.htmlToDom(result.ditaToc.text, {xmlMode: true});
								while (ditaTocDom && !(ditaTocDom.name && ditaTocDom.name.toLowerCase() === "map")) {
									ditaTocDom = ditaTocDom.next;
								}
								if (ditaTocDom) {
									ditaTocs[index] = common.domToHtml(ditaTocDom, {xmlMode: true});
								} else {
									logger.warning("Excluded from DITA TOC file because file TOC does not contain a <map>: " + destinationPath);
								}
							}
						} else {
		...
		if (tocOrder) {
			...
			/* do a pass for external links */
			for (var i = 0; i < tocOrder.length; i++) {
				var match = REGEX_LINK.exec(tocOrder[i]);
				if (match) {
					if (tocXML && !xmlTocs[i]) {
						var root = common.htmlToDom(tocXMLadapter.createRoot(), {xmlMode: true});
						var topic = common.htmlToDom(tocXMLadapter.createTopic(match[2], match[1]));
						common.domUtils.appendChild(root, topic);
						xmlTocs[i] = common.domToHtml(root, {xmlMode: true});
					}
					if (tocDITA && !ditaTocs[i]) {
						var root = common.htmlToDom(tocDITAadapter.createRoot(), {xmlMode: true});
						var topic = common.htmlToDom(tocDITAadapter.createTopic(match[2], match[1]));
						common.domUtils.appendChild(root, topic);
						ditaTocs[i] = common.domToHtml(root, {xmlMode: true});
					}
				}
			}
