...
function generate(options, logger) {
	...
	var generateHTML = function(source, destination) {
		...
		try {
			execSync("rm -rf " + fs.realpathSync(tempDirPath));
		} catch (e) {
			if (e.code !== "ENOENT") {
				/* the path exists, fall back to attempting to delete every file within it */
				var filenames = fs.readdirSync(tempDirPath);
			...
			if (stat.isDirectory()) {
				...
				if (extension === EXTENSION_MARKDOWN) {
					...
					var writeTempTOCfile = function(extension, content) {
						...
						try {
							var writeTempTOCFd = fs.openSync(tempTOCpath, "w");
							var success = common.writeFile(writeTempTOCFd, new Buffer(content));
							fs.closeSync(writeTempTOCFd);
							if (!success) {
						...
						if (match) {
							...
							if (fs.existsSync(entryFile)) {
								...
								if (stat.isDirectory()) {
									...
									logger.warning("Excluded from toc files because points at a non-markdown file: " + tocOrderPath + "#" + tocOrder[i]);
								}
							} else {
