		...
		for (var i = 0; i < filenames.length; i++) {
			...
			if (stat.isDirectory()) {
				...
				if (extension === EXTENSION_MARKDOWN) {
					...
					var htmlOutput = result.html.text;

					var dom = common.htmlToDom(htmlOutput);
					...
					var htmlElement = domUtils.find(function(node) {return node.name && node.name.toLowerCase() === "html";}, [dom], true, 1);
