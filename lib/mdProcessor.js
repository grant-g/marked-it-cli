...
var FILENAME_TOC_ORDER = "toc.txt";
...
function generate(options, logger) {
	...
	function traverse_tree(source, destination) {
		...
		if (!disableTOC) {
			/* first look for a toc.txt file in this directory */
		...
		for (var i = 0; i < filenames.length; i++) {
			...
			if (stat.isDirectory()) {
				...
				if (!disableTOC) {
					var childTocPath = path.join(destPath, FILENAME_TOC_XML);

					try {
						stat = fs.statSync(childTocPath);
						if (stat.isFile()) {
							var outputPath = path.join(current, FILENAME_TOC_XML);
							var linkElementText = '<toc><link href="' + outputPath + '" /></toc>\n';
							if (!tocOrder) { /* no specified order */
								xmlTocs.push(linkElementText);
							} else {
								var index = tocOrder.indexOf(current);
								if (index !== -1) {
									xmlTocs[index] = linkElementText;
								} else {
									logger.warning("Excluded link to toc.xml because not present in the parent's toc.txt: " + childTocPath);
								}
							}
						}
					} catch (e) {
						/* a toc.xml was not written in the subfolder, so nothing to link to */
					}					
				}
			} else {
				...
				if (extension === EXTENSION_MARKDOWN) {
					...
					if (disableTOC) {
						...
						options.filePath = outputFilename;
					}
					...
					if (!disableTOC) {
						...
						if (result.htmlToc) {
							tocFilename = path.join(destination, current.replace(EXTENSION_MARKDOWN_REGEX, EXTENSION_HTML_TOC));
							try {
								writeTOCFd = fs.openSync(tocFilename, overwrite ? "w" : "wx");
								var htmlTocText = pd.xml(result.htmlToc.text);
								success = common.writeFile(writeTOCFd, new Buffer(htmlTocText));
								fs.closeSync(writeTOCFd);
								if (!success) {
									logger.error("Failed to write: " + tocFilename);
								} else {
						...
						if (result.ditamap) {
							...
							try {
								...
								var ditamapText = pd.xml(result.ditamap.text);
								success = common.writeFile(writeDitamapFd, new Buffer(ditamapText));
								fs.closeSync(writeDitamapFd);
								if (!success) {
...
////						}
						...
						if (result.xmlToc) {
							if (!tocOrder) {
								xmlTocs.push(result.xmlToc.text); /* no specified order */
							} else {
								index = tocOrder.indexOf(current);
								if (index !== -1) {
									xmlTocs[index] = result.xmlToc.text;
								} else {
									logger.warning("Excluded from toc.xml because not present in toc.txt: " + destinationPath);
								}
							}

							var xmlTocFilename = path.join(destination, current.replace(EXTENSION_MARKDOWN_REGEX, EXTENSION_XML_TOC));
							try {
								var writeXmlTocFd = fs.openSync(xmlTocFilename, overwrite ? "w" : "wx");
								var xmlTocText = pd.xml(result.xmlToc.text);
								success = common.writeFile(writeXmlTocFd, new Buffer(xmlTocText));
								fs.closeSync(writeXmlTocFd);
								if (!success) {
									logger.error("Failed to write: " + xmlTocFilename);
								} else {
									var errors = result.xmlToc.errors;
									if (errors) {
										var errorString = "{\n";
										errors.forEach(function(current) {
											errorString += "\t" + current + "\n";
										...
										errorString += "}";
										logger.warning("Wrote: " + xmlTocFilename + " with the following errors:\n" + errorString);
									} else {
										logger.info("Wrote: " + xmlTocFilename);
									}
								}
							} catch (e) {
								logger.error("Failed to open file to write: " + xmlTocFilename + "\n" + e.toString());
							}
						}
					}
		...
		if (mdFileProcessed && !disableTOC) {
			var xmlTocFilename = path.join(destination, FILENAME_TOC_XML);
			if (xmlTocs.length) {
				var tocDom = common.htmlToDom("<toc></toc>\n");
				var domUtils = htmlparser.DomUtils;
				xmlTocs.forEach(function(current) {
					if (current) {
						var currentDom = common.htmlToDom(current, {xmlMode: true});
						var children = domUtils.getChildren(currentDom);
						if (children) {
							children.forEach(function(child) {
								domUtils.appendChild(tocDom, child);
							});
						}
					}
				});
				var tocString = common.domToHtml(tocDom, {xmlMode: true});

				var canWriteXmlToc = true;
				try {
					stat = fs.statSync(xmlTocFilename);
					/* there's already a colliding file, so be careful */
					canWriteXmlToc = overwrite && stat.isFile();
				} catch (e) {
				...
				if (canWriteXmlToc) {
					try {
						var writeTocFd = fs.openSync(xmlTocFilename, overwrite ? "w" : "wx");
					...
					logger.warning("Skipped writing toc.xml file due to a file collision: " + xmlTocFilename);
				}
			} else {
				logger.warning("Skipped writing toc.xml file because no files in the directory are included: " + xmlTocFilename);
			}
		}
	}
