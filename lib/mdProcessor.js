				...
				if (tocOrder) {
					...
					try {
						...
						if (stat.isFile()) {
							...
							if (index !== -1) {
								xmlTocs[index] = htmlTocs[index] = ditaTocs[index] = linkElementText;
							} else {
				...
				if (extension === EXTENSION_MARKDOWN) {
					...
					if (tocOrder) {
						...
						if (index !== -1) {
							...
							logger.warning("Excluded from toc files because not present in toc.txt: " + destinationPath);
						}
					}
